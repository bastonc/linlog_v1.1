...
from PyQt5.QtWidgets import QApplication, QWidget, QMainWindow, QAction, QTableWidget, QTableWidgetItem, QTextEdit, QLineEdit,QPushButton, QLabel, QVBoxLayout, QHBoxLayout, QComboBox
...
class logWindow (QWidget):
    ...
    def addRecord(self,recordObject):
        ...
        #record to file
        stringToAdiFile = "<BAND:"+str(len(recordObject['BAND']))+">"+recordObject['BAND']+"<CALL:"+str(len(recordObject['CALL']))+">"

        stringToAdiFile = stringToAdiFile + recordObject['CALL']+"<FREQ:"+str(len(recordObject['FREQ']))+">"+recordObject['FREQ']
        stringToAdiFile = stringToAdiFile + "<MODE:"+str(len(recordObject['MODE']))+">"+recordObject['MODE']+"<OPERATOR:"+str(len(recordObject['OPERATOR']))
        stringToAdiFile = stringToAdiFile + ">"+recordObject['OPERATOR']+"<QSO_DATE:"+str(len(recordObject['QSO_DATE']))+">"
        stringToAdiFile = stringToAdiFile + recordObject['QSO_DATE']+"<TIME_ON:"+str(len(recordObject['TIME_ON']))+">"
        stringToAdiFile = stringToAdiFile + recordObject['TIME_ON']+"<RST_RCVD:"+str(len(recordObject['RST_RCVD']))+">"+recordObject['RST_RCVD']
        stringToAdiFile = stringToAdiFile + "<RST_SENT:"+str(len(recordObject['RST_SENT']))+">"+recordObject['RST_SENT']+"<COMMENTS:"+str(len(recordObject['COMMENTS']))+">"+recordObject['COMMENTS']+"<TIME_OFF:6>000000<eQSL_QSL_RCVD:1>Y<EOR>\n\n"
...
class logForm(QMainWindow):
 ...
 def initUI(self):
        ...
        self.inputCall.textChanged[str].connect(self.onChanged) # событие изминения текста, привязываем в слот функцию onChanged
        self._filter = Filter()
        # adjust for your QLineEdit
        self.inputCall.installEventFilter(self._filter)
        self.inputCall.returnPressed.connect(self.logFormInput) # событие нажатия Enter, привязываем в слот функцию logSettings
        ...
        style="QTextEdit{background:"+settingsDict['form-background']+"; border: 1px solid "+settingsDict['solid-color']+";}"
 ...
 def onChanged (self, text):
     ...
     if re.search('[А-Я]', text):
            ...
            style= "QLineEdit{ border: 1px solid "+settingsDict['solid-color']+"; border-radius: 50px; background: "+settingsDict['form-background']+"; font-weight: bold;}"
 ...
 def logFormInput (self):
     ...
     if call!='':
        ...
        #print("'QSO_DATE':'20190703', 'TIME_ON':'124600', 'FREQ':"+freq+" 'CALL':"+cal+"'MODE'"+mode+" 'RST_RCVD':"+rstR+" 'RST_SENT':"+rstS+", 'NAME':"+name+", 'QTH':"+qth+"'OPERATOR':"+operator+"'BAND':"+band+"'COMMENT':"+comment)
         ...
         recordObject={'QSO_DATE':date, 'TIME_ON':time, 'FREQ':freq, 'CALL':call, 'MODE':mode, 'RST_RCVD':rstR, 'RST_SENT':rstS, 'NAME':name, 'QTH':qth, 'OPERATOR':operator, 'BAND':band, 'COMMENTS':comment}
        ...
        while 1:
            ...
            if output_data != '':
                ...
                if output_data[0:2].decode('utf-8') == "DX":
                    ...
                    if len(cleanList) > 4:
                        self.telnetCluster.tableWidget.setItem(lastRow - 1, 0, QTableWidgetItem(cleanList[3]))
...
class telnetCluster (QWidget):
    ...
    def initUI(self):
           ...
           self.setGeometry(100, 320, 300, 220)
           ...
           self.tableWidget.setHorizontalHeaderLabels(["   Freq   ", " Call ", "Other"])
           self.tableWidget.resizeColumnsToContents()
           self.tableWidget.resizeRowsToContents()
           #self.tableWidget.resizeColsToContents()
